...
public class JSONAPIHook extends Hook {
    ...
    public void onEnable() {
        ...
        if ((grab != null) && (grab instanceof JSONAPI)) {
			...
			((JSONAPI) grab).registerMethod("{   \"name\": \"isFakedOffline\",   \"desc\": \"Checks if named player is faked offline\",   \"call\": \"Plugins.VanishNoPacket.getManager().isFakedOffline(0)\",  \"returns\": [\"boolean\", \"True if player is faked offline. False if offline or visible.\"],   \"args\": [       [\"String\", \"Player's name\"]   ]}");
